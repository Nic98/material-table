import MaterialTableRubberMeta from 'F:\\Mywebsites\\material-table\\material-table\\lowcode\\material-table-rubber\\meta.ts'
...
function fillRealVersion(meta, packageName = 'material-table-rubber', version = '0.1.0', basicLibraryVersion={"@alifd/next":"1.25.23","@alifd/meet":"2.6.3","antd":"4.17.3"}) {
  if (!meta || !version) {
    return meta;
  }
  const { npm } = meta;
...
const componentMetas = [MaterialTableRubberMeta];
const components = [];
const npmInfo = {};
...
if (!execCompile) {
  window.MaterialTableRubberMeta = { components, componentList };
}